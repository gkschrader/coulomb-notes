...
\begin{example}
Take $N=\g$ to be the adjoint representation and $U=\mathfrak{n}=[\mathfrak{b},\mathfrak{b}]\subset \g$ to be the Lie algebra of the unipotent radical of $\mathfrak{b}$. Then $X_U\simeq T^*(G/B)$ is the cotangent bundle of $G/B$.
\end{example}

We'd like to incorporate these $B$-representations $U$, or equivalently the $G$-equivariant vector bundles $X_U$, into the convolution story somehow. But we need to be careful here,  since -- being vector bundles -- the varieties $X_U$ themselves are not proper.  Also recall that by the Thom isomorphism, we can identify 
$$
K_G(X_U)\simeq K_G(G/B)\simeq K_T(\pt),
$$
so at least as modules over $K_G(\pt)$ we don't get anything new from the $X_U$ alone. 


But where the $X_U$ really come into their own is that, as well as being a vector bundles over $G/B$, they come with a $G$-equivariant `action map' to the $G$-module $N$:
$$
\mu: X_U\rightarrow N, \quad [g,u]B\mapsto g\cdot u.
$$
...
\end{example}
The action map gives us another way to view $X_U$ as
$$
X_U = \left\{ (gB,v)\in G/B\times N ~\big|~ v\in gU\subset N \right\}.
$$

We can use the proper maps $\mu$ to form fiber products:
$$
X_{U_1}\times_N X_{U_2}=\{(g_1B,g_2B,v)\in G/B\times G/B\times N~\big|~ v\in g_1U_1\cap g_2U_2\}.
$$
Even if we're only interested in $K_G(X_U)$ for a single $U$, the ability to vary $U$ will still be useful.
 
The fiber product $X_{U_1}\times_N X_{U_2}$ maps $G$-equivariantly to $G/B\times G/B$, but will generally no longer be a vector bundle: and over the pre-image of the orbit $O_w$ through $(gB,B)\in G/B\times G/B$ it is a vector bundle with fiber modeled on $wU_1\cap U_2$, i.e the $G$-equivariant vector bundle on $O_w$ induced from the $H=B\cap wBw^{-1}$-module $wU_1\cap U_2$. In particular, by the cellular fibration lemma $K_G(X_{U_1}\times_N X_{U_2})$ is a free module over $K_G(\pt)$ of rank $|W|^2$.



Suppose $M_1,M_2$ are smooth, proper $G$-varieties with equivariant vector bundles $E_1,E_2$. Since $E_1,E_2$ are not proper, we can't run the convolution story with them directly since pushforward from $E_1\times E_2$ will not be well-defined. On the other hand, suppose that we have a closed $G$-subvariety $i:Z\hookrightarrow E_1\times E_2$ (maybe not smooth) with the property that the restricted projection
$$
\overline{\pi}_2\colon Z\rightarrow E_1\times M_2
$$
is proper. Then $\pr_{E_1}\circ i:Z\rightarrow E_1$ is proper, and so for $\mathcal{M}\in K_G(Z)$ and $\alpha\in K_G(E_2)$ we get a well-defined convolution
$$
\mathcal{M}*\alpha = (\pr_{E_1}\circ i)_*\left(\mathcal{M}\otimes i^*\pr_{E_2}^*\alpha\right)\in K_G(E_1).
$$
In particular, we can apply this  setup to 
$$
Z=Z_{U_1,U_2}=X_{U_1}\times_N X_{U_2}\subset X_{U_1}\times X_{U_2},
$$
since in this case the map 
$$
\overline{\pi}_2 \colon X_{U_1}\times_N X_{U_2}\rightarrow X_{U_1}\times G/B, \quad (g_1B,g_2B,v)\mapsto ((g_1B,v),g_2).
$$ 
is actually just a closed embedding. 
...
\end{proof}
In our case, we have $M_1=M_2=G/B$ and the left vertical arrow is injective since $\overline{\pi}_2$ is a closed embedding and $z^*_1:K_G(X_{U_1})\simeq K_G(G/B)$ is the Thom isomorphism. In particular, the representation of $K_G(Z)$ on $K_G(G/B)$ coming from the diagram is \emph{faithful.}


Recall that we analyzed the convolution algebra for $N=0$ by computing the action of classes of sheaves supported on the $G$-orbit closures $\overline{O}_1=O_1,\overline{O}_{s_i}\subset G/B\times G/B$ for the identity element and the simple reflections. The preimage $\pi^{-1}O_1$ in $Z_{U_1,U_2}$ is the vector bundle on the diagonally embedded $G/B\rightarrow (G/B)^2$ with fiber $gU_1\cap gU_2$ over $(gB,gB)$. Its image under $\overline{\pi}_2$ is a vector bundle on the diagonal of the zero section in $X_{U_1}\times G/B$. Hence $\mathcal{O}_{\pi^{-1}O_1}$ has a Koszul resolution by $\pi^*[U_1/(U_1\cap U_2)]^\vee$, where we write $U_1/(U_1\cap U_2)$ for the $G$-equivariant vector bundle on the orbit induced by the $B$-rep $U_1/(U_1\cap U_2)$ (i.e. having the latter as its fiber over the identity coset.) 
...
$$
Recalling the description $\overline{O}_{s_i}=G/B\times_{G/P_i}G/B$, we see that in the above we have $g_1P_i=g_2P_i$ and hence the image under $\overline{\pi}_2$ of $\pi_{W_i,W_i}^{-1}\left(\overline{O}_{s_i}\right)$ is actually the entire space $X_{W_i}\times G/B$. So
$$
...
We can rewrite the story in the small model for convolution where we work equivariantly for $B$ rather than $G$ as follows. If we set
$$
R_U = \{(gB,s)\in G/B\times N~\big|~ s\in gU_2\cap U_1\},
$$
then
$$
G\times ^B R_U\simeq X_{U_1}\times_N X_{U_2}, \quad [g_1,(g_2B,s)] \mapsto (g_1B,g_1g_2B, g_1s).
$$
